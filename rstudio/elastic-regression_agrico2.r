...
setwd("C:/Users/jsjch/OneDrive/Documents/github/emission-regression-analysis-all/energy_data")

# Load the datasets
oil_energy_use <- read_csv("Energy use (kg of oil equivalent per capita).csv")
renewable_energy <- read_csv("Renewable energy consumption (% of total final energy consumption).csv")
agri_emissions <- read_csv("Emissions_Agricultural_Energy use.csv")

# Data Preparation
# Identify year columns for filtering
year_columns_oil_energy_use <- colnames(oil_energy_use)[grepl("^\\d{4}$", colnames(oil_energy_use))]
year_columns_renewable_energy <- colnames(renewable_energy)[grepl("^\\d{4}$", colnames(renewable_energy))]

# Filter columns to include only 1990 to 2014
year_columns_oil_energy_use <- year_columns_oil_energy_use[year_columns_oil_energy_use >= "1990" & year_columns_oil_energy_use <= "2014"]
year_columns_renewable_energy <- year_columns_renewable_energy[year_columns_renewable_energy >= "1990" & year_columns_renewable_energy <= "2014"]

# Reshape datasets to long format using the filtered columns
oil_energy_use_long <- oil_energy_use %>%
  pivot_longer(
    cols = all_of(year_columns_oil_energy_use),
    names_to = "year",
    values_to = "value"
  )

renewableEnergy_long <- renewableEnergy %>%
  pivot_longer(cols = all_of(year_columns_renewable_energy),
               names_to = "Year",
...
oilEnergyUse_agg <- oilEnergyUse_long %>%
  group_by(Year) %>%
  summarize(Oil_Use = mean(Energy_Use, na.rm = TRUE))
